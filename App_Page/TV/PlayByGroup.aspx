<%@ Page   Language="C#" MasterPageFile="~/App_MasterPage/TvMasterPage.master" AutoEventWireup="true" CodeFile="PlayByGroup.aspx.cs" Inherits="App_Page_TV_PlayByGroup" %>
...
<asp:Content ID="Content4" ContentPlaceHolderID="DefaultContentPlaceHolder" runat="Server">
    ...
    <asp:SqlDataSource ID="DefaultSqlDataSource" runat="server" ConnectionString="<%$ ConnectionStrings:ConnectionString %>" SelectCommand="SELECT TOP 10 VideoTitle,VideoId,ID FROM [News-TV] WHERE (GroupID = @GroupID) ORDER BY ID DESC">
        <SelectParameters>
    ...
    <asp:Panel ID="ShowAllPanel" runat="server" Visible="False">
        ...
        <asp:SqlDataSource ID="ShowAllSqlDataSource" runat="server" ConnectionString="<%$ ConnectionStrings:ConnectionString %>" SelectCommand="SELECT ID,VideoTitle,Date,Likes FROM [News-TV] WHERE (GroupID = @GroupID) ORDER BY [ID] DESC">
            <SelectParameters>
                <asp:QueryStringParameter Name="GroupID" QueryStringField="GroupID" />
            </SelectParameters>
        </asp:SqlDataSource>
