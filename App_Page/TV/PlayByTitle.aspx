<%@ Page   Language="C#" MasterPageFile="~/App_MasterPage/TvMasterPage.master" AutoEventWireup="true" CodeFile="PlayByTitle.aspx.cs" Inherits="App_Page_TV_PlayByTitle" %>
...
<asp:Content ID="Content4" ContentPlaceHolderID="DefaultContentPlaceHolder" Runat="Server">
    ...
    <asp:SqlDataSource ID="DefaultSqlDataSource" runat="server" ConnectionString="<%$ ConnectionStrings:ConnectionString %>" SelectCommand="SELECT TOP 10 VideoTitle,VideoId,ID FROM [News-TV] WHERE (VideoTitle = @VideoTitle) ORDER BY ID DESC">
        <SelectParameters>
            <asp:QueryStringParameter Name="VideoTitle" QueryStringField="Title" />
        </SelectParameters>
    </asp:SqlDataSource>
    ...
    <asp:Panel ID="ShowAllPanel" runat="server" Visible="False">
        ...
        <asp:SqlDataSource ID="ShowAllSqlDataSource" runat="server" ConnectionString="<%$ ConnectionStrings:ConnectionString %>" SelectCommand="SELECT ID,VideoTitle,Date,Likes FROM [News-TV] WHERE (VideoTitle = @VideoTitle) ORDER BY [ID] DESC">
            <SelectParameters>
                <asp:QueryStringParameter Name="VideoTitle" QueryStringField="Title" />
            </SelectParameters>
        </asp:SqlDataSource>
