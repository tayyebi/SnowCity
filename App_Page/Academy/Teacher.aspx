<%@ Page Language="C#" MasterPageFile="~/App_MasterPage/AccademyMasterPage.master" AutoEventWireup="true" CodeFile="Teacher.aspx.cs" Inherits="App_Page_Academy_Teacher" %>
...
<asp:Content ID="Content4" ContentPlaceHolderID="CPH_Default" Runat="Server">
    ...
    <asp:SqlDataSource ID="DefaultSqlDataSource" runat="server" ConnectionString="<%$ ConnectionStrings:ConnectionString %>" SelectCommand="SELECT TOP 10 ArticleID,Title,Abstract FROM [Academy-Article] WHERE (UserID = @UserID) ORDER BY ArticleID DESC">
        <SelectParameters>
    ...
    <asp:Panel ID="ShowAllPanel" runat="server" Visible="False">
        ...
        <asp:SqlDataSource ID="ShowAllSqlDataSource" runat="server" ConnectionString="<%$ ConnectionStrings:ConnectionString %>" SelectCommand="SELECT Title,ArticleID,Title,Likes,Date FROM [Academy-Article] WHERE (UserID = @UserID) ORDER BY [ArticleID] DESC">
            <SelectParameters>
                <asp:QueryStringParameter Name="UserID" QueryStringField="UserID" />
            </SelectParameters>
        </asp:SqlDataSource>
