...
namespace WBrowser
{
    public partial class WBrowser : Form
    {
        ...
        {
           Application.Run(new SnowBrowser.SplashScreen());
        }
        public WBrowser()
        {
...
//delete history
        ...
        public static Image favicon(String u, string file)
        {
            Uri url = new Uri(u);
                ...
                {
                    return SnowBrowser.Properties.Resources.BrowserBar;
                }
...
//show history in tree wiew
        private void showHistory()
        {
            ...
            if (File.Exists(historyXml))
            {
                ...
                if (comboBox1.Text.Equals("مرور امروز را نشان بده"))
                {
                    ...
                    foreach (XmlElement el in myXml.DocumentElement.ChildNodes)
                    {
                        ...
                        node.ToolTipText = el.GetAttribute("url") + "\nآخرین مرور: " + el.GetAttribute("lastVisited") + "\nدفعات مرور: " + el.GetAttribute("times");
                        node.Name = el.GetAttribute("url");
                        node.ContextMenuStrip = histContextMenu;
           ...
           if (comboBox1.Text.Equals("بر اساس سایت نشان بده"))
           {
           ...
           if (comboBox1.Text.Equals("بر اساس تاریخ نشان بده"))
           {  
               historyTreeView.ShowRootLines = true;
               historyTreeView.Nodes.Add("2 Weeks Ago","دو هفته قبل",2,2);
               historyTreeView.Nodes.Add("Last Week","هفته قبل",2,2);
               historyTreeView.Nodes.Add("This Week","همین هفته",2,2);
               historyTreeView.Nodes.Add("Yesterday","دیروز",2,2);
               historyTreeView.Nodes.Add("Today","امروز",2,2);
               foreach (XmlElement el in myXml.DocumentElement.ChildNodes)
               {
                  DateTime d=DateTime.Parse(el.GetAttribute("lastVisited"),currentCulture);
...
//internet options
        private void internetOptionsToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            if (intOp.ShowDialog() == DialogResult.OK)
            {
                ...
                linkBar.BackColor = Color.White;
                adrBar.BackColor = Color.White;
            }


        }
        private void تنظیماتاینترنتToolStripMenuItem_Click(object sender, EventArgs e)
        {
